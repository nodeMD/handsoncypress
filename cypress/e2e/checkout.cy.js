...
describe("Checkout tests", () => {
  it("user is able to finish checkout", () => {
    cy.visit("/");
    cy.fillAndSubmitLoginForm(validUser.email, validUser.password);

    // `user is able to see list of products is which has length of ${desiredNumberOfItems} items`
    cy.get(inventoryPage.containers.item).should(
      "have.length",
      desiredNumberOfItems
    );

    // "user is able to add random item to cart then check if cart icon shows proper number and check if the proper price is shown on cart page"
    ...
    // "user is unable to continue without filling checkout form with proper data"
    cy.clickContinue();
    ...
    // "user is able to continue after filling checkout form with proper data"
    cy.fillPostalCode(fakeUser.postalCode);
    cy.clickContinue();
    cy.url().should("include", "/checkout-step-two");

    // `user is able to see that the shipping information is '${shippingMessage}'`
    cy.get(checkoutPage.labels.summary)
      .contains(shippingMessage)
      .should("be.visible");

    // `user is able to see confirmation message: '${thankYouMessage}'`
